...
def bbody_fit(wave, flux, flux_error = 1):
	#if flux_error is None:
	#	flux_error = 0.2 * flux
	#lflux = log10(flux)
	#flux_error = 1 #log10(flux_error)
	chi2best = 1e300
	best = None
	for Twave in numpy.logspace(2, 4, 40):
		flux_model = wave**-5 / (numpy.exp(1/(wave/Twave)) - 1)
		s = (flux * flux_model / flux_error**2).sum() / (flux_model**2 / flux_error**2).sum()
		chi2 = (((flux - s * flux_model) / flux_error)**2).sum()
		if chi2 < chi2best:
			chi2best = chi2
			best = [Twave, s]
	...
	return best + [slope, chi2best - chi2]
...
for object_id, object_data in e.groupby(e.index.get_level_values(0)):
	...
	for passband in bands:
		...
		r = plt.errorbar(x=time / (1 + z) - all_time.min(), y=flux / all_flux.max(), yerr=flux_error / all_flux.max(), linestyle='-', marker='x')
	
	if peaktimes:
		tpeak_lo, tpeak_hi, tpeak = min(peaktimes), max(peaktimes), numpy.median(peaktimes)
	else:
		tpeak_lo, tpeak_hi, tpeak = None, None, all_time.values[all_flux.values.argmax()] / (1 + z)
	Trise = []
	Tfall = []
	lastt = -100
	deltat = 0.1
	...
	for t in numpy.unique(all_time[numpy.logical_and(all_flux > 0, is_detected)]):
		...
		if (all_passband[mask] == 0).any() and (all_passband[mask] == 1).any():
			blueratioseries.append([t, all_flux[mask][all_passband[mask] == 0] / all_flux[mask][all_passband[mask] == 1]])
		if (all_passband[mask] == 4).any() and (all_passband[mask] == 5).any():
			redratioseries.append([t, all_flux[mask][all_passband[mask] == 4] / all_flux[mask][all_passband[mask] == 5]])
		
		if mask.sum() < 3:
			continue
		#times = all_time[mask]
		passband = all_passband[mask].values
		plt.figure('SEDevol')
		norm = 10**((trest - tpeak)/20.)
		if norm > 1000 or norm < 0.001:
			norm = numpy.nan
		#print('       norm:', norm, trest, tpeak)
		#norm = 1.
	...
	if mask.sum() > 6:
		...
		phi_rise, theta_rise, evolrise = fit_logplanetilt(all_time[mask].values - all_time[mask].values.max() / (1 + z), 
			log10(wavelengths[passbands] / (1 + z) / 300),
			log10(all_flux[mask].values / passband_efficiencies[passbands]))
		phi_rise, theta_rise = fit_logplane(all_time[mask].values - all_time[mask].values.min() / (1 + z), 
			log10(wavelengths[passbands] / (1 + z) / 300),
			log10(all_flux[mask].values / passband_efficiencies[passbands]))
	else:
	...
	if mask.sum() > 6:
		passbands = all_passband[mask]
		phi_fall, theta_fall, evolfall = fit_logplanetilt(all_time[mask].values - all_time[mask].values.min() / (1 + z), 
			log10(wavelengths[passbands] / (1 + z) / 300),
			log10(all_flux[mask].values / passband_efficiencies[passbands]))
		phi_fall, theta_fall = fit_logplane(all_time[mask].values - all_time[mask].values.min() / (1 + z), 
			log10(wavelengths[passbands] / (1 + z) / 300),
			log10(all_flux[mask].values / passband_efficiencies[passbands]))
	else:
	...
	if len(peakfluxes) > 2:
		...
		norm = s * Twave**-5 / (numpy.exp(1/(wavenorm/Twave)) - 1)
		plt.plot(Twave, norm, 'x', ms=2, color=color)
		n += 1
