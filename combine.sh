...
paste -d, training_set_metadata.csv training_set_std_features.txt training_set_colorslope_features.txt|sed 's,#,,g'|awk -F, '$8==0 || NR==1'|gzip > training_set_gal.csv.gz
paste -d, training_set_metadata.csv training_set_std_features.txt training_set_colorslope_features.txt|sed 's,#,,g'|awk -F, '$8!=0 || NR==1'|gzip > training_set_exgal.csv.gz
